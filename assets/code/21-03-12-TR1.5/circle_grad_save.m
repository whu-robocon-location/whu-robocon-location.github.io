...
x= 0;                     %走180°以后小车x坐标的改变量，初值是机械组量的模型参数
y = 107.7669323;                %走180°以后小车x坐标的改变量，初值是机械组量的模型参数

K1_POS = -0.258478351 + x;    %正转180°时解算出来的K1的值。注意，这里中的K对应的实际上是ARC_K1_POS
K2_POS = 48.23118261 + y;       %正转180°时解算出来的K2的值
K1_NEG = -0.209530036 + x;    %反转180°时解算出来的K1的值
K2_NEG = 47.95673966 +  y;       %反转180°时解算出来的K2的值
...
h = 100;                           %积分步长，根据实验，取较大积分步长时仍有较好的收敛
max_iter_num = 300;             %最大迭代次数
...
L_POS = (sqrt((K1_POS - X)*(K1_POS - X) +( K2_POS - Y)*(K2_POS- Y)) / 2) / 15;
L_NEG = (sqrt((K1_NEG - X)*(K1_NEG - X) +( K2_NEG - Y)*(K2_NEG- Y)) / 2) / 15;
    ...
    if ( x>x_min && x<x_max && y>y_min && y<y_max)
        ...
        fprintf("当前迭代次数为%d\n",iter_num);
         ...
         if(f_next<=f_now)
              ...
              circle(3) = subs(L_POS*15,{X,Y},{x,y});
              circle(4) = subs(L_NEG*15,{X,Y},{x,y});
         end
    else
        h = h/10;
        x = x_last - h*fx_now;
