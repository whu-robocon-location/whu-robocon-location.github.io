...
void kal_flt_init_acc_x(k_filter *k_flt) {   
    int i = 0; 
    /* 系统状态转移矩阵 */
    k_flt->A = 1;
    /* 观测矩阵，行矩阵 */
    ...
    while (i < MAX_NUM) 
	{
		comb_flt.data[i] = g_output_info.accel_x;
		i++;
    }
		/* 系统状态预测矩阵，列矩阵 */
    k_flt->X_last = g_output_info.accel_x;

    /* 观测噪声 */
    k_flt->R = 0.000001;//0.1;
    /* 过程噪声 */
    k_flt->Q = 0.000001;//0.1
    /* 初始量测值, 即Z(k)为0 */
    k_flt->input = 0;
}
void kal_flt_init_acc_y(k_filter *k_flt) {   
    int i = 0; 
    /* 系统状态转移矩阵 */
    k_flt->A = 1;
    /* 观测矩阵，行矩阵 */
    ...
    while (i < MAX_NUM) 
	{
		comb_flt.data[i] = g_output_info.accel_y;
		i++;
    }
		/* 系统状态预测矩阵，列矩阵 */
    k_flt->X_last = g_output_info.accel_y;
