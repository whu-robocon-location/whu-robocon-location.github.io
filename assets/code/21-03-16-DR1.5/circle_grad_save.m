...
%%                                       赋值区域
x = -93.079559;% -93.1;                 %走180°以后小车x坐标的改变量，初值是机械组量的模型参数
y = 23.658180;%23.61818;                %走180°以后小车x坐标的改变量，初值是机械组量的模型参数

K1_POS = -7.23790026 + x;       %正转180°时解算出来的K1的值。注意，这里中的K对应的实际上是ARC_K1_POS
K2_POS = -9.4043932 + y;        %正转180°时解算出来的K2的值
K1_NEG = -7.13233232 + x;       %反转180°时解算出来的K1的值
K2_NEG = -9.26155376 + y;       %反转180°时解算出来的K2的值
...
h = 1000000;                   %积分步长，取大积分步长。如fx_now数量级为n次，h数量级为2-n
max_iter_num = 50;             %最大迭代次数
error_permitted = 0.1;        %允许的最大装配误差
scale = 6;                     %L/α
%%                                      函数部分
