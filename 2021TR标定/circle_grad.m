...
y = -98.12045;                %走180°以后小车x坐标的改变量，初值是机械组量的模型参数

K1_POS = 0.475318819 + x;    %正转180°时解算出来的K1的值。注意，这里中的K对应的实际上是ARC_K1_POS
K2_POS = 9.662813507 + y;       %正转180°时解算出来的K2的值
K1_NEG = 0.153768617 + x;    %反转180°时解算出来的K1的值
K2_NEG = 9.70980014 + y;       %反转180°时解算出来的K2的值

a_pos_period = -pi;              %α由tan函数解算出来，所以需要加周期。假设α实际为90°，正转解算出来是-90°，那就令α_pos_period = pi
a_neg_period = -pi;               %假设α实际为90°，反转解算出来是-90°，那就令α_neg_period = pi，一般来说，neg和pos，一个是±pi，另一个就是0

% 梯度下降(更新x，y)参数修改

h = 10000;                           %积分步长，根据实验，取较大积分步长时仍有较好的收敛
max_iter_num = 20;             %最大迭代次数
